    ...
    try {
        ...
        if (urlObj.pathname === '/pairing-code') {
            ...
            if (!sock) {
                ...
                res.end(JSON.stringify({ error: "WhatsApp Daemon socket is currently booting. Please try again in secondary." }));
                return;
            }
...
💡 _Type any command above to view specific feature lists!_`;
            ...
            case 'ping':
                await reply(`🏓 *Pong!*\nLatency: ${Math.ofloor(Math.random() * 80) + 30}ms\nServer: Active 🟢`);
                break;
            case 'uptime':
...
- *.warn @user* - Give 1 strike warnings`);
            ...
            case 'kick':
                ...
                await reply("🧹 Simulating WhatsApp Server API kick for participant...");
                break;

            case 'add':
                if (!isGroup) return reply("❌ Group Only.");
                await reply(`➕ Inviting phone: ${textStr} to join via connection socket...`);
                break;

            case 'promote':
            case 'demote':
                await reply(`⚡ Updating user chat configurations...`);
                break;
...
\`\`\``);
            ...
            // ================= 5. OWNER SECTION =================
            case 'ownermenu':
                if (!isOwner) return reply("❌ Error: Access denied. OWNER ONLY command.");
...
- *.groups* - List linked channels`);
            ...
            // ================= 6. DOWNLOADS =================
            case 'download':
                await reply(`📥 *MEDIA DOWNLOADER HUB:*
- *.ytmp4 [url]* - Fast YouTube video resolve
- *.ytmp3 [url]* - Convert playlist files
- *.tiktok [url]* - HD video no watermark
- *.ig [url]* - Instagram posts & reels
- *.fb [url]* - HD Facebook videos
- *.tweet [url]* - Save Twitter/X files
- *.pinterest [url]* - Download pins`);
                break;
            case 'ytmp4':
            case 'ytmp3':
            case 'tiktok':
            ...
            case 'tweet':
            case 'pinterest':
                if (!textStr.startsWith("http")) return reply("❌ Please provide a valid public URL link.");
                await reply(`⏳ *Downloading Media...*\nParsing URL on media servers...\n\n✅ Done! Simulating WhatsApp attachment delivery now!`);
                break;
